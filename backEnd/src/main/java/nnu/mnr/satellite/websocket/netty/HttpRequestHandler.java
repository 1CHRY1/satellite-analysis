...
public class HttpRequestHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private final WsActionDispatch websocketActionDispatch;

    public HttpRequestHandler(WsActionDispatch websocketActionDispatch) {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) {
        ...
        websocketActionDispatch.dispatch(request.uri(), WsAction.HAND_SHAKE, ctx.channel());
        ...
        if (handshaker == null) {
            ...
            handshaker.handshake(ctx.channel(), request).addListener(future -> {
                if (future.isSuccess()) {
                    websocketActionDispatch.dispatch(request.uri(), WsAction.OPEN,ctx.channel());
                } else {

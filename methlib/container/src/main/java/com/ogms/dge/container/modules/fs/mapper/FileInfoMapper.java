...
public interface FileInfoMapper extends BaseMapper<FileInfo> {
     ...
     * @return
     */
    @Select("SELECT IFNULL(SUM(file_size), 0) FROM container_file WHERE user_id = #{userId}")
     ...
     * @return
     */
    @Select("select "+ colms +" from container_file where user_id = #{userId} and file_id = #{fileId}")

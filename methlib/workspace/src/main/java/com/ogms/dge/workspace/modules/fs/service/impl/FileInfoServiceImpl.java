package com.ogms.dge.workspace.modules.fs.service.impl;
...
import com.ogms.dge.workspace.common.exception.BusinessException;
import com.ogms.dge.workspace.common.utils.Constant;
import com.ogms.dge.workspace.common.utils.StringTools;
import com.ogms.dge.workspace.config.AppConfig;
...
import com.ogms.dge.workspace.modules.fs.entity.enums.*;
...
import com.ogms.dge.workspace.modules.fs.mapper.FileInfoMapper;
...
import static com.ogms.dge.workspace.common.utils.FileUtils.*;
...
//            updateUserSpace(webUserDto, totalSize);
    ...
    @Override
//    @Transactional(rollbackFor = Exception.class)
    public List<String> handleSaveToDb(Long userId, String tmpFilePath, String outputFilePath, String fileFrontName, String outputFileRealName, String newFilePid, String fileGroup) throws IOException {
        ...
        // 拷贝真正文件到用户数据文件夹
        com.ogms.dge.workspace.common.utils.FileUtils.copyDirectory(Paths.get(tmpFilePath), Paths.get(outputFilePath));
        ...
        if (files != null) {
            for (File file : files) {
                try {
                    ...
                    String fileId = saveOutputFile(userId, Paths.get(Objects.toString(userId), fileName).toString(), fileFullFrontName, newFilePid, fileGroup, fileTypeEnum, fileMd5, fileSize);
                    fileIds.add(fileId);
                    return fileIds;
    ...
    private String saveOutputFile(Long userId, String filePath, String fileFullFrontName, String newFilePid, String fileGroup, FileTypeEnums fileTypeEnum, String fileMd5, Long fileSize) {
        // 单输出源 TODO一个输出源对应一个fileGroup
        FileInfo fileInfo = new FileInfo();
    ...
    public void handleShp(FileInfo existingFile) {
        ...
        String shpName = com.ogms.dge.workspace.common.utils.FileUtils.getFileNameWithoutSuffix(existingFile.getFileName());
        ...
        for (int j = 0; j < fileInfoList.size(); j++) {
            ...
            String newPath = file.getFilePath().substring(0, file.getFilePath().lastIndexOf("/") + 1)
                    + shpRealName + "." + com.ogms.dge.workspace.common.utils.FileUtils.getFileSuffix(file.getFilePath());
    ...
    public File downloadFile(String fileUrl, String saveDir, boolean isUuid) throws IOException {
        ...
        // 应当防止重名
        if (isUuid)
            fileName = UUID.randomUUID() + "." + com.ogms.dge.workspace.common.utils.FileUtils.getFileSuffix(fileName);

...
def _add_ray_decorators(code):
    ...
    for line in lines:
        if line.strip().startswith('def ') and not line.strip().startswith('def main'):
            ...
            remote_functions.append(func_name)
        ...
        for func_name in remote_functions:
            if f'{func_name}(' in line:
                ...
                if '=' in line and not line.strip().startswith('#'):
                    # 使用更精确的正则表达式
                    pattern = rf'(\s*)([^=]+)\s*=\s*{func_name}\s*\('
                    replacement = rf'\1\2 = ray.get({func_name}.remote('
                    
                    if re.search(pattern, line):
                        modified_line = re.sub(pattern, replacement, line)
                        # 在行尾添加ray.get的闭合括号
                        if modified_line != line and modified_line.rstrip().endswith(')'):
                            modified_line = modified_line.rstrip() + ')'
                        elif modified_line != line:

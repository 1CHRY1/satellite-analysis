...
import { GetESRGAN, GetSuperResolution, GetSuperResolutionV2 } from '@/api/http/satellite-data/visualize.api'
import {
    visualLoad,
    selectedSensor,
    selectedRBand,
...
import { getGridESRGANUrl, getGridSuperResolutionUrlV2 } from '@/api/http/interactive-explore'
        ...
        if (visualLoad.value) {
            ...
            try {
                ...
                try {
                    ...
                    console.log('超分返回数据', bandres.data)
                    // console.log(result.value)
                    const url = getGridESRGANUrl(calTask.value.taskId, gridData.value)
                    GridExploreMapOps.map_addGridESRGANLayer(gridData.value, url)
                    // bus.emit(
                    //     'SuperResTimeLine',
                    //     {
                    //         data: bandres.data,

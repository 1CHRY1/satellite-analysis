...
import { getVectorContinuousAttr, getVectorDiscreteAttr } from '@/api/http/satellite-data'
...
export const useGridVector = () => {
    ...
    const handleCheckAllChange = (tableName: string, val: boolean) => {
        ...
        const attrList = gridVectorSymbology.value[tableName].checkedAttrs.map((item) => {
            const targetAttr = gridVectorSymbology.value[tableName].attrs.find(
                (i) => i.label === item,
            )
            return targetAttr
        })
        GridExploreMapOps.map_updateGridMVTLayerStyle(
            attrList as any,
            gridVectorSymbology.value[tableName].selectedField,
            selectedGrid.value as GridData,
            gridVectorSymbology.value[tableName].selectedFieldType,
        )
    }
    const handleCheckedAttrsChange = (tableName: string, value: string[]) => {
        ...
        gridVectorSymbology.value[tableName].isIndeterminate =
    ...
    const handleVectorVisualize = () => {
        ...
        previewIndex.value = gridData.value.vectors.findIndex(
        ...
        GridExploreMapOps.map_addGridMVTLayer(
            tableName,
            url,
            attrList as any,
            gridVectorSymbology.value[tableName].selectedField,
    ...
    const getAttrs4CustomField = async (tableName: string, field: string | undefined) => {
        ...
        let fields = gridData.value.vectors.filter((vector) => vector.tableName == tableName)[0].fields
        if (fields.continuous.includes(field)) {
            ...
            gridVectorSymbology.value[tableName].checkedAttrs = result.map(r => `${r?.low}-${r?.up}`)
            gridVectorSymbology.value[tableName].selectedFieldType = 'continuous'
            ...
            gridVectorSymbology.value[tableName].selectedFieldType = 'discrete'
        }

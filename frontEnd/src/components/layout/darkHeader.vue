    ...
    <header class="flex items-center justify-between px-10 py-2 shadow-md select-none">
        <div class="flex flex-3/5 items-center justify-start">
            <div class="flex items-center">
                ...
                <a class="bg-gradient-to-r from-sky-100 to-white bg-clip-text font-semibold whitespace-nowrap text-transparent sm:text-lg md:text-xl lg:text-2xl"
                    href="home">
                    {{ t('nav.title') }}</a>
            </div>
            ...
            <nav class="ml-[5vw] flex items-center justify-center">
                <template v-for="(item, index) in navItems" :key="item.path || index" class="space-x-[1.5vw]">
                    <!-- 分隔线 (移到前面) -->
                    <div v-if="index > 0" class="h-8 w-0 border-r-2 border-gray-600 mx-[1.5vw]">
                    </div>

                    <!-- 有子菜单的项 -->
                    <div v-if="item.children" class="relative group">
                        <div class="text-shadow-deepblue relative rounded-md px-2 py-1 font-semibold whitespace-nowrap text-white transition-all duration-300 hover:text-sky-100 sm:text-lg md:text-lg lg:text-xl cursor-pointer flex items-center"
                            @mouseenter="openSubMenu(index)" @mouseleave="closeSubMenu">
                            <a :href="item.path">
                                {{ item.name }}
                            </a>
                        ...
                        <transition name="menu-fade">
                            <ul v-show="activeSubMenu === index"
                                ...
                                <li v-for="child in item.children" :key="child.path">
                                    <router-link v-if="child.path && !child.disabled" :to="child.path"
                    ...
                    <!-- 普通菜单项 -->
                    <template v-else>
                        <a v-if="item.external" :href="item.path" target="_blank" rel="noopener noreferrer"

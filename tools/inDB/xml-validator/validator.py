...
def process_folder(PRODUCT_CONFIG, scenes_dir):
    ...
    for i, folder in enumerate(folders):
        CURRENT_FOLDER = copy.deepcopy(folder)
        ...
        if len(xml_files) > 1:
            ...
            exit_with_error(f"[ERROR][{i + 1}][{CURRENT_FOLDER}] No XML file found in {CURRENT_FOLDER}\r\n")
        ...
        if XML_EXISTS:
            ...
            # 解析xml文件
            SCENE_CONFIG = parse_xml(xml_path, SCENE_CONFIG, CURRENT_FOLDER, i)
        ...
        if SCENE_CONFIG["SCENE_PATH"] is None:
            ...
            if not os.path.exists(folder_path):
                exit_with_error(f"[ERROR][{i + 1}][{CURRENT_FOLDER}]: Path not exist: {folder_path}")
            
            # 获取所有子文件夹
            subfolders = [f for f in os.listdir(folder_path) 
                        if os.path.isdir(os.path.join(folder_path, f))]
            if not subfolders:
                exit_with_error(f"[ERROR][{i + 1}][{CURRENT_FOLDER}] No scene file found in {folder_path}\r\n")
            else:
                ...
                if not tif_files:
                    exit_with_error(f"[ERROR][{i + 1}][{CURRENT_FOLDER}]: No tif files found in {target_folder_path}")
        ...
        if XML_EXISTS:
            ...
            for field in field_list:
                if SCENE_CONFIG["XML"][field] is None:
                    exit_with_error(f"[ERROR][{i + 1}][{CURRENT_FOLDER}] {field} is not set in {xml_path}\r\n")
            image_time = SCENE_CONFIG["XML"]["IMAGE_TIME"]
            try:
                datetime.strptime(image_time, "%Y-%m-%d %H:%M:%S")
            except Exception:
                exit_with_error(f"[ERROR][{i + 1}][{CURRENT_FOLDER}] IMAGE_TIME format error: {image_time}, should be yyyy-mm-dd hh:mm:ss, file: {xml_path}\r\n")
...
def parse_xml(xml_path, SCENE_CONFIG, CURRENT_FOLDER, index):
    try:
        # 解析XML文件
        ...
        if start_time is not None and start_time.text and check_time_format(start_time.text):
            ...
            exit_with_error(f"[ERROR][{index}][{CURRENT_FOLDER}] No time found in {xml_path} or time format error, should be yyyy-mm-dd hh:mm:ss\r\n")
        # Get cloud info
        ...
        def get_coords(lat_elem, lon_elem):
            ...
            if lat is not None and lon is not None and lat.text and lon.text:
                ...
                exit_with_error(f"[ERROR][{index}][{CURRENT_FOLDER}] No {lat_elem} or {lon_elem} found in {xml_path}\r\n")
        ...
        exit_with_error(f"[ERROR][{index}][{CURRENT_FOLDER}] Error parsing XML file: {e}\r\nXML file: {xml_path}\r\n")
    finally:
        return SCENE_CONFIG
...
# 命令行参数
parser = argparse.ArgumentParser(description='Validate the xml files')

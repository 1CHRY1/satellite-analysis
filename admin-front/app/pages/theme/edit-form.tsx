...
import { updateScene as updateTheme } from "~/apis/https/scene/scene.admin";
...
const editTheme = async (values: Theme) => {
	console.log(values);
...
export const EditThemeButton: React.FC<{
	onSuccess: () => void | Promise<any>;
	initTheme: Theme;
	sensorOpts: { label: string; value: string }[];
}> = ({ onSuccess, initTheme, sensorOpts }) => {
				...
				fields: [
					{
						...
						type: "text",
						rules: [
							{ required: true, message: "请输入栅格产品标识" },
						],
					},
					{
						name: "sensorId",
			...
			trigger={
				<Tooltip title="编辑">
					<Button
						...
						shape="circle"
						onClick={() => {
							getAllProduct([initTheme.sensorId]);
						}} // onClick可以同时作为初始化和trigger
						icon={<EditOutlined />}
					/>
				</Tooltip>
			}
			initialValues={initTheme}
			schema={schema}
			onFinish={async (values) => {
				values.sceneId = initTheme.sceneId;
				values.sceneTime = dayjs(values.sceneTime).format(
					"YYYY-MM-DDTHH:mm:ss",
				);
				console.log(values);
				const result = await editTheme(values);
				if (result) onSuccess();
				return result;
			}}
